...
class ModelVersion(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    artifact_format: Mapped[str | None] = mapped_column(Text(), nullable=True)
    feature_schema_version: Mapped[str | None] = mapped_column(Text(), nullable=True)
    feature_schema_hash: Mapped[str | None] = mapped_column(CHAR(64), nullable=True)
    target_version: Mapped[str | None] = mapped_column(Text(), nullable=True)
    runtime_contract_version: Mapped[str | None] = mapped_column(Text(), nullable=True)
    model_card_uri: Mapped[str | None] = mapped_column(Text(), nullable=True)
