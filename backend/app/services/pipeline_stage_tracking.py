...
A thin context manager so `extraction_service.run_extraction` and
`verification_service.run_verification` each record one attempt row per
invocation — start time, attempt number, and either a `SUCCEEDED` or
`FAILED` outcome — without duplicating the bookkeeping in both services.
Document-level idempotency (skipping a stage whose document status has
already moved on) still lives in each service's own entry guard; this is
purely the observability/attempt-history record NFR-3 asks for.
"""
